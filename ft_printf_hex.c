...
/*   Updated: 2026/02/18 19:36:55 by agaspary         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

/* int	ft_puthex_bc(unsigned long n, int upper)
{
	int		count;
	char	*base;
	...
	if (n >= 16)
	{
		count += ft_puthex_bc(n / 16, upper);
		count += ft_putchar_bc(base[n % 16]);
	}
	else
		count += ft_putchar_bc(base[n]);
	return (count);
} */
